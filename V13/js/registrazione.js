...
(function () {
    ...
    function validaUsername() { let v = campi.username.input ? campi.username.input.value.trim() : ''; if (!REGEX_USERNAME.test(v)) { mostraErrore(campi.username, 'Username non valido (inizia con lettera, 3-50 caratteri).'); return false; } mostraErrore(campi.username, ''); return true; }
    function validaPassword() { let v = campi.password.input ? campi.password.input.value : ''; aggiornaForza(v); if (!REGEX_PASSWORD.test(v)) { mostraErrore(campi.password, 'Password non valida (8-16 car., maiusc., minusc., numero, spec.).'); return false; } mostraErrore(campi.password, ''); return true; }
