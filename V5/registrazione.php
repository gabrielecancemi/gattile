...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errori)) {
        ...
        $connI = null;
        try {
            // Verifica unicità username (reader)
            $connR = getDB('reader');
            $check = mysqli_prepare($connR, 'SELECT id FROM utenti WHERE username = ? LIMIT 1');
            if (!$check) throw new RuntimeException(mysqli_error($connR));
            ...
            if ($exists) {
                ...
                // Inserimento (registrator — solo INSERT)
                ...
                if (!$ins) throw new RuntimeException(mysqli_error($connI));
            ...
            if ($connR) mysqli_close($connR);
            if ($connI) mysqli_close($connI);
