...
/* ── Hashing password: SHA-256 con salt unico per utente ──────────
 *
 * Non possiamo modificare lo schema del DB (unica colonna "password"),
 * quindi salviamo salt e digest insieme nel formato:
 *     sha256$<salt_esadecimale>$<digest_esadecimale>
 * dove digest = SHA-256(salt . password). Il salt (16 byte casuali) è
 * unico per ogni utente e generato con random_bytes().
 */

/** Crea l'hash da memorizzare a partire dalla password in chiaro. */
function creaHashPassword(string $password): string
{
    $salt   = bin2hex(random_bytes(16));            // 32 caratteri esadecimali
 ...
 * (formato sha256$salt$digest). Usa hash_equals per confronto
 * a tempo costante (evita timing attack).
 */
function verificaHashPassword(string $password, string $memorizzato): bool
{
    $parti = explode('$', $memorizzato);
    if (count($parti) !== 3 || $parti[0] !== 'sha256') {
        return false;
    }
    $salt   = $parti[1];
    $atteso = $parti[2];
    $calcolato = hash('sha256', $salt . $password);
    return hash_equals($atteso, $calcolato);
}
...
function tentaLogin(string $username, string $password): array
{
    ...
    if (str_starts_with($hash, 'sha256$')) {
        // Formato corrente: SHA-256 con salt unico per utente
        $ok = verificaHashPassword($password, $hash);
        mysqli_close($conn);
    } elseif (str_starts_with($hash, '$2y$') || str_starts_with($hash, '$2b$')) {
        // Legacy bcrypt: verifica e poi migra a SHA-256+salt
        $ok = password_verify($password, $hash);
        $daMigrare = $ok;
        mysqli_close($conn);
    } else {
        // Password in chiaro (DB demo): confronto diretto poi migra
        $ok = hash_equals($hash, $password);
        $daMigrare = $ok;
        mysqli_close($conn);
    }

    // Migrazione trasparente al formato SHA-256+salt
    if ($daMigrare) {
        $connMod = getDB('modifier');
        if ($connMod) {
            $upd = mysqli_prepare($connMod, 'UPDATE utenti SET password = ? WHERE id = ?');
            if ($upd) {
                $nuovoHash = creaHashPassword($password);
                $uid = (int) $utente['id'];
                mysqli_stmt_bind_param($upd, 'si', $nuovoHash, $uid);
                mysqli_stmt_execute($upd);
                mysqli_stmt_close($upd);
                error_log('[auth] Password migrata a SHA-256+salt per utente id=' . $uid);
            }
