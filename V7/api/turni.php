...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $utenteCorrente = utenteLoggato();
    $utenteId = $utenteCorrente ? (int) $utenteCorrente['id'] : 0;
    ...
    while ($data <= $fine) {
        if ((int)$data->format('w') !== 0 && $data > $now) {
            ...
            // Verifica se l'utente corrente e' gia' iscritto a questa fascia
            $giaIscritto = false;
            if ($utenteId > 0) {
                $chk = mysqli_prepare(
                    $conn,
                ...
                if ($chk) {
                    mysqli_stmt_bind_param($chk, 'is', $utenteId, $iso);
                    mysqli_stmt_execute($chk);
                    $resChk = mysqli_stmt_get_result($chk);
                    $giaIscritto = (mysqli_fetch_row($resChk) !== null);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($fasceRichieste as $fascia) {
        ...
        if (!mysqli_stmt_execute($ins)) {
            error_log('[turni POST] Execute insert: ' . mysqli_stmt_error($ins));
