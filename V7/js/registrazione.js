...
(function () {
    ...
    var forzaMeter = document.getElementById('forza-password');
    var forzaTesto = document.getElementById('forza-password-testo');
    var progresso  = document.getElementById('progresso-form');
    var progrTesto = document.getElementById('progresso-testo');
    var btnSubmit  = document.getElementById('btn-registra');

    var REGEX_PWD = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^a-zA-Z\d]).{8,16}$/;
    var REGEX_USR = /^[a-zA-Z][a-zA-Z0-9_]{2,49}$/;
    var ETICHETTE  = ['Molto debole', 'Debole', 'Sufficiente', 'Buona', 'Ottima'];

    function mostraErrore(info, msg) {
        if (!info || !info.errore) return;
        if (msg) {
            info.errore.textContent = '⚠ ' + msg;
    ...
    function validaNome()    { var v = campi.nome.input ? campi.nome.input.value.trim() : ''; if (v.length < 2) { mostraErrore(campi.nome, 'Nome: almeno 2 caratteri.'); return false; } mostraErrore(campi.nome, ''); return true; }
    function validaCognome() { var v = campi.cognome.input ? campi.cognome.input.value.trim() : ''; if (v.length < 2) { mostraErrore(campi.cognome, 'Cognome: almeno 2 caratteri.'); return false; } mostraErrore(campi.cognome, ''); return true; }
    function validaInd()     { var v = campi.indirizzo.input ? campi.indirizzo.input.value.trim() : ''; if (v.length < 5) { mostraErrore(campi.indirizzo, 'Indirizzo: almeno 5 caratteri.'); return false; } mostraErrore(campi.indirizzo, ''); return true; }
    function validaUser()    { var v = campi.username.input ? campi.username.input.value.trim() : ''; if (!REGEX_USR.test(v)) { mostraErrore(campi.username, 'Username non valido (inizia con lettera, 3-50 caratteri).'); return false; } mostraErrore(campi.username, ''); return true; }
    function validaPwd()     { var v = campi.password.input ? campi.password.input.value : ''; aggiornaForza(v); if (!REGEX_PWD.test(v)) { mostraErrore(campi.password, 'Password non valida (8-16 car., maiusc., minusc., numero, spec.).'); return false; } mostraErrore(campi.password, ''); return true; }
    function validaConf()    { var v = campi.conferma.input ? campi.conferma.input.value : ''; var p = campi.password.input ? campi.password.input.value : ''; if (!v) { mostraErrore(campi.conferma, 'Conferma la password.'); return false; } if (v !== p) { mostraErrore(campi.conferma, 'Le due password non coincidono.'); return false; } mostraErrore(campi.conferma, ''); return true; }
    ...
    form.addEventListener('submit', function (e) {
        var ok = [validaNome(), validaCognome(), validaInd(), validaUser(), validaPwd(), validaConf()];
        if (ok.indexOf(false) !== -1) {
            e.preventDefault();
            var primo = Object.keys(campi).find(function (k) { return campi[k].errore && !campi[k].errore.hidden; });
            if (primo && campi[primo].input) campi[primo].input.focus();
        }
    });

    aggiornaProgresso();
    console.log('[Registrazione] Script caricato');
})();
