 ...
 * Validazione lato client (JS) + server.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Riempi i campi per ripopolare il form
    $campi = compact('nome', 'cognome', 'indirizzo', 'username');

    // Validazioni server (ridondanti rispetto al JS, ma indispensabili)
    $errori = [];
    ...
    // Username: inizia con lettera, lunghezza 3-50
    if (!preg_match('/^[a-zA-Z][a-zA-Z0-9_]{2,49}$/', $username)) {
        $errori[] = 'Username non valido: deve iniziare con una lettera e contenere solo lettere, numeri e underscore (3-50 caratteri).';
    }

    // Password: 8-16 car, almeno 1 maiusc, 1 minusc, 1 numero, 1 spec.
    if (!preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^a-zA-Z\d]).{8,16}$/', $password)) {
        $errori[] = 'La password deve avere 8-16 caratteri e includere almeno una maiuscola, una minuscola, un numero e un carattere speciale.';
    }
    ...
    if (empty($errori)) {
        try {
            // Usa "registratore": ha solo INSERT su utenti
            $db = getDB('registrator');

            // Verifica unicità username (con "reader" per non sprecare privilegi)
...
stampaTesta(
    'Registrazione',
    'Registrati a Gattile San Paolo per prenotare visite o turni di volontariato.',
    'registrazione.php'
);
echo '<body>';
stampaHeader();
        ...
        <fieldset>
            ...
            <label for="reg-username" class="campo-obbligatorio">
                ...
                <input
                    ...
                    id="reg-username"
                    name="username"
                    value="<?= esc($campi['username']) ?>"
                    autocomplete="username"
                    required
                    aria-required="true"
                    aria-describedby="aiuto-reg-username"
                    maxlength="50"
                    spellcheck="false"
            ...
            <label for="reg-password" class="campo-obbligatorio">
                ...
                <input
                    ...
                    required
                    aria-required="true"
                    aria-describedby="aiuto-reg-password"
                    minlength="8"
                    maxlength="16"
                >
                ...
                <meter
                    ...
                    value="0"
                    aria-label="Forza della password"
                    title="Forza password: da 0 (debole) a 4 (ottima)"
                ></meter>
